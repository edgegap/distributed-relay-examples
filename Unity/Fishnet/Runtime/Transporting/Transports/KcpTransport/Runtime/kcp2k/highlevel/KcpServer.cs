...
namespace kcp2k
{
    public class KcpServer
    {
        ...
        public virtual bool TryStart(string addressIPv4, string addressIPv6, ushort port)
        {
            // only start once
            if (socket != null)
            {
            ...
            // listen
            if (!KcpServerUtils.BindServerSocket(config.DualMode, addressIPv4, addressIPv6, port, out socket))
            {
                return false;
            }
